...
stocks
 ├─ stock_aliases
 ├─ stock_prices_daily                   # 최근 30~90일 차트용 DB 저장
 ├─ external_data_cache_files            # 장기 주가/원본 응답 파일 인덱스
...
현재 구현은 키움 일봉 수집 후 최근 90거래일을 저장하고, 화면에서는 보통 최근 30일을 조회한다.
...
- 장기 데이터: external_data_cache_files가 가리키는 로컬 JSON/CSV/Parquet 파일 사용
- 차트 요청 시:
...
|---|---:|---|---|
| price_daily | 삼성전자 | data-cache/prices/005930/daily.json | 2023-12-11 ~ 2026-06-02 |
| stock_basic_info | 삼성전자 | data-cache/prices/005930/basic-info.json | 수집일 기준 |
| dart_raw | 삼성전자 | data-cache/dart/005930/2024/annual.json | 2024 사업보고서 |
| news_raw | 삼성전자 | data-cache/news/005930/YYYY-MM-DD.json | 수집일 기준 뉴스 |
...
| 필수 | external_data_cache_files | 대용량 캐시 파일 인덱스 |
...
| 확장 테이블 | 이유 |
|---|---|
| stock_prices_daily | 최근 차트 빠른 조회 |
...
6. DB의 external_data_cache_files를 갱신한다.
7. 최근 30~90일 데이터만 stock_prices_daily에 선택 저장한다.
...
|---|---|
| 종목 기본 정보 | stocks |
| 현재가/등락률 | stock_prices_daily 또는 로컬 주가 캐시 |
...
| DART 원본 확인 | external_data_cache_files |
...
| 원본 응답 재처리 | external_data_cache_files |
...
ai_chat_messages
external_api_fetch_logs
stock_prices_daily            # 선택: 최근 30~90일만
