...
> 현재 상태: **KOSPI/KOSDAQ 전체 종목 마스터 seed → 전체 종목 검색 → 삼성전자 기준 DART 재무·키움 주가·네이버 뉴스·규칙 기반 신호등·Gemini 설명 → 요약·상세·관심종목·AI 질문 화면까지 MVP 흐름 연결 완료**
> 실행 방법: **프로젝트 루트에서 `npm start` → `http://127.0.0.1:4000` 접속**
> 다음 목표: **삼성전자 분석 데모 흐름 검증 → 발표 시나리오 정리 → 다른 종목에도 DART/주가/뉴스/AI 분석 워커를 일반화**
...
현재 종목 마스터와 검색은 KOSPI/KOSDAQ 전체 종목을 대상으로 한다.
외부 API 비용과 구현 범위를 고려해 재무·주가·뉴스·AI 분석 데이터 수집 워커는 삼성전자 기준으로 먼저 검증한다.

추천 개발 순서:

```text
KOSPI/KOSDAQ 전체 종목 마스터 seed
→ 전체 종목 검색 기능 검증
→ 삼성전자 기준 분석·캐시·화면 흐름 완성
→ 다른 종목에도 수집/분석 워커 일반화
```
...
현재는 KOSPI/KOSDAQ 전체 종목 마스터와 삼성전자 기준 분석 데모 데이터가 들어가며, 백엔드 API와 프론트 화면 연결까지 완료된 상태다.
...
```text
프론트엔드와 백엔드를 독립 실행할 수 있다.
프로젝트 루트의 npm start로 백엔드 API와 프론트 정적 파일을 하나의 서버에서 실행할 수도 있다.
...
현재 종목 마스터와 검색 대상은 KOSPI/KOSDAQ 전체 종목이다.
KRX KIND 상장법인 목록에서 KOSPI/KOSDAQ 종목을 가져오고, OpenDART corpCode 목록으로 `dart_corp_code`를 매칭한다.

실제 분석·캐시·화면 데모는 삼성전자(`005930`)를 기준으로 먼저 검증한다.
...
```text
stocks 테이블에 KOSPI/KOSDAQ 전체 종목이 들어간다.
각 종목은 stock_id로 식별된다.
stock_aliases에는 기본 별칭으로 회사명과 종목코드가 들어간다.
```

현재 완료 내용:

```text
KOSPI/KOSDAQ 전체 종목 seed 워커를 구현했다.

실행 명령:
cd backend
npm run seed:stocks:krx          # dry-run
npm run seed:stocks:krx -- --apply

워커 동작:
- KRX KIND에서 KOSPI/KOSDAQ 상장법인 목록 수집
- OpenDART corpCode 목록으로 dart_corp_code 매칭
- stocks에 stock_code, ticker, company_name_ko, market, dart_corp_code, listed_at 저장
- stock_aliases에 회사명과 종목코드 기본 별칭 저장
...
```text
DART 수집 및 캐시 저장 검증 성공
- corp_code=00126380
- fiscalYear=2024
- reprt_code=11011
- status=000
- itemCount=213
- data-cache/dart/005930/2024/annual.json 저장
- external_data_cache_files 메타데이터 저장
```
...
현재 구현에서는 키움 일봉 수집 결과를 사용해 현재가, 등락률, 최근 30거래일 주가 그래프까지 표시한다.
데이터가 없을 때는 화면에서 수집 안내 또는 빈 상태를 보여준다.
...
뉴스 분석은 현재 구현 완료된 확장 기능이다.
네이버 검색 API로 최근 뉴스를 수집하고 Gemini로 종목 영향 신호를 저장한다.
...
## Step 19-2. 전체 종목 검색 및 분석 확장 테스트

확인 사항:

```text
KOSPI/KOSDAQ 전체 종목 seed 후 검색이 정상 동작하는지
회사명과 종목코드 별칭 검색이 정상 동작하는지
...
- [x] DB 테이블 생성
- [x] metric seed 입력
- [x] KOSPI/KOSDAQ 전체 종목 입력 워커 구현
- [x] stock_aliases 입력 (전체 종목 회사명/종목코드 기준, 삼성전자 추가 별칭은 개발용 seed에 포함)
...
현재 상태에서 가장 먼저 해야 할 일은 **Phase 19. 삼성전자 분석 데모 흐름 검증 및 다른 종목 분석 워커 일반화 준비**다.
...
2. 발표용 사용자 시나리오 정리
3. 한계점 및 확장 방향 정리
4. 다른 종목에도 DART/주가/뉴스/AI 분석 워커를 적용할 수 있게 일반화
```
