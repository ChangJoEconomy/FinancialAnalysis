...
> 현재 상태: **삼성전자 기준 검색 → DART 재무 수집 → 지표 계산 → 규칙 기반 신호등 → Gemini 설명 → 요약분석 화면 → 재무상세 화면 표시까지 MVP 흐름 연결 완료**
...
현재 상태에서 가장 먼저 해야 할 일은 **Phase 14. 관심종목 기능 구현**이다.

바로 다음 순서:

```text
1. 관심종목 추가 상태 조회
2. 요약분석 화면에서 추가/삭제 토글 구현
3. 관심종목 목록 화면 구현
4. 최신 전체 신호와 AI 한 줄 요약 표시
5. 메모와 정렬 기능 연결
```

이 단계가 끝나면 다음으로 AI 질문 UI를 확장한다.
